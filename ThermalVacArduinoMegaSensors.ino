// Maybe delete the unused libraries to clear memory?
#include <SPI.h>
#include <Wire.h>
...
//----THIS MUST MATCH YOUR FLIGHT CODE-----
...
//Alternately, use a hard serial line if the microcontroller supports it --> Ask Garrett
//XBee xBee = XBee(&Serial1, ID);
/1111  */
#define SCREEN_WIDTH 128 // OLED display width, in pixels
#define SCREEN_HEIGHT 64 // OLED display height, in pixels

#define chipSelect 4 // Adafruit Wireless SD shield
...
void setup() {
  ...
  //configure radio with ID; designate as flight unit
  ...
  String header = "PR    T1 T2 T3 T4 Sec";  //setup data format, and print it to the monitor and SD card
...
// Check is=f the thermocouples are SPI or I2C (If I2C, can use the same "rails" or connections, just address each one)
void loop() {
...
void SDLogging(String P, int T1, int T2, int T3, int T4, bool &TakeData) {
    if (SDactive/* && TakeData==true*/) {
      ...
      datalog.println(P + " " + T1 + " " + T2 + " " + T3 + " " + T4 + " " + String(count));          //...and to SD card if available // CHange Format Later
